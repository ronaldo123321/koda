...
protected `read-only`/`workspace-write`, and denied network. C3A is implemented:
strict TypeScript/Rust declarations, public evidence and error codes, bounded
value-free catalog normalization/digests, exact-byte streaming redactors, and
shared binary fixtures now agree across languages. C3B adds trusted
