...
function commandStartFailure(
  ...
  if (
    snapshot.failure?.code === "RESOURCE_LIMIT_APPLY_FAILED" ||
    snapshot.failure?.code === "RESOURCE_LIMIT_INTEGRITY_LOST"
  ) {
    return new CommandError(snapshot.failure.code, snapshot.failure.message);
  }
  const code =
