...
Koda exposes two structured write tools to the selected model. `apply_patch` creates or exactly updates one UTF-8 text file. `apply_changes` coordinates up to 16 independent creates, ordered exact updates, same-filesystem moves, or deletions under one approval. A change set is fully prepared and previewed before mutation, revalidated under a workspace-scoped writer lease, and compensating rollback runs after ordinary failure or cancellation. If rollback cannot prove that it is undoing Koda's own bytes, the result is explicitly uncertain and must be inspected rather than automatically repeated.
...
```json
{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":8,"client":{"name":"my-koda-client","version":"0.1.0"}}}
