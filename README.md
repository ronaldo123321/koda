...
The Phase 3 local-agent foundation, Phase 4A crash-safe workspace recovery, and Phase 4B supervised native execution are complete. Koda now has an opt-in Rust execution supervisor with a versioned local protocol, reconnectable job observation, POSIX process groups, Windows Job Objects and ConPTY, bounded retained output, and explicit native capability reporting on top of those foundations:
...
- An Ink `koda-chat` REPL that uses app-server v13 exclusively for sequential chat, approvals, thread browsing, durable search, windowed history navigation, runtime settings, artifact, context, Plan, extension, activity, and mutation-recovery inspection, Stage acceptance, and resume.
...
- Credential-free protocol v13 `extension/catalog`, `extension/read`, and `thread/extensions`, direct CLI inspection, and idle-only Ink `/extensions` with current-versus-historical labeling.
...
```json
{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":13,"client":{"name":"my-koda-client","version":"0.1.0"}}}
