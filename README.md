...
The Phase 3 local-agent foundation, Phase 4A crash-safe workspace recovery, Phase 4B supervised native execution, Phase 4C1 execution policy/reporting, Phase 4C2A macOS Seatbelt delivery, and Phase 4C2B1/C2B2 Linux contract and real capability probe are complete. A verified macOS native executor advertises and enforces protected Pipe/PTY execution. On Linux, Koda now discovers only trusted fixed Bubblewrap paths, freezes binary identity, constructs a read-only mount/user/network namespace plus Koda-owned seccomp bootstrap, and advertises schema v3 only after a real multi-profile startup self-test; the pre-job gate intentionally keeps protected Linux user commands disabled until C2B3 connects that verified path to Pipe/PTY execution and durable applied evidence. Native protocol v4, durable format v4, legacy compatibility, grant binding, and evidence-derived client projection remain shared across the platform work, while Windows sandboxing stays deferred until the Linux path is complete. Koda has an opt-in Rust execution supervisor with a versioned local protocol, reconnectable job observation, POSIX process groups, Windows Job Objects and ConPTY, bounded retained output, explicit native capability reporting, and end-to-end execution-policy evidence on top of those foundations:
