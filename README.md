...
The Phase 3 local-agent foundation is complete through Phase 3H. Phase 3H adds bounded scoped project Skills, reviewed declarative command templates, atomic dynamic MCP tool generations, a transactional out-of-process plugin lifecycle, and protocol/client inspection with durable activation, refresh, and recovery evidence on top of the Phase 3G planning Harness:
...
- An Ink `koda-chat` REPL that uses app-server v12 exclusively for sequential chat, approvals, thread browsing, durable search, windowed history navigation, runtime settings, artifact, context, Plan, extension, and activity inspection, Stage acceptance, and resume.
...
- App-server v12 `plan/get` and exact live `plan/acceptance/resolve`, plus CLI and Ink `/plan`, acceptance, rejection-feedback, and recovery views.
...
- Credential-free protocol v12 `extension/catalog`, `extension/read`, and `thread/extensions`, direct CLI inspection, and idle-only Ink `/extensions` with current-versus-historical labeling.
...
```json
{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":12,"client":{"name":"my-koda-client","version":"0.1.0"}}}
